/* Shirwal Circle interactions: improved login, cart, checkout->WhatsApp */
const KEY_CART = 'shirwal_circle_cart';
const KEY_USER = 'shirwal_circle_user';
const SAMPLE_USER = { username: 'demo@shirwalcircle.in', password: 'Shirwal@123' };
...
function bindLogin(){
  ...
  form.addEventListener('submit',e=>{
    ...
    if(username!==SAMPLE_USER.username||password!==SAMPLE_USER.password){ help.textContent='Invalid credentials. Try sample login shown below.'; return; }
    localStorage.setItem(KEY_USER, JSON.stringify({ username, loggedAt:new Date().toISOString() }));
    location.href='account.html';
  });
}

function renderLoginState(){
  const box=document.querySelector('[data-login-state]'); if(!box)return;
  const u=JSON.parse(localStorage.getItem(KEY_USER)||'null');
  box.innerHTML=u?`<span class="badge success">Logged in as ${u.username}</span>`:'<span class="badge warning">Not logged in</span>';
}
...
function bindScrollTheme(){
  const root=document.documentElement;
  window.addEventListener('scroll',()=>{ const p=Math.min(window.scrollY/(document.body.scrollHeight-window.innerHeight||1),1); root.style.setProperty('--scrollGlow', (0.2+p*0.8).toFixed(2)); });
}

document.addEventListener('DOMContentLoaded',()=>{ updateCartCount(); bindAddButtons(); renderCartPage(); bindLogin(); renderLoginState(); bindCheckoutWhatsapp(); bindScrollTheme(); });
